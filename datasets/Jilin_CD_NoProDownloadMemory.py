...
def read_RSimages(mode, read_list=False):
    ...
    #         label = Color2Index(io.imread(label_path))
            ...
            data_A.append(img_A_path)
            data_B.append(img_B_path)
...
class RS(data.Dataset):
    def __init__(self, mode, random_crop=False, crop_nums=6, sliding_crop=False, crop_size=512, random_flip=False, half=False):
    ...
    def __getitem__(self, idx):
        ...
        data_A = read_img(self.data_A[idx])
        data_B = read_img(self.data_B[idx])
        label = Color2Index(io.imread(self.labels[idx]))
        ...
        if self.half:
            return F.to_tensor(data_A).half(), F.to_tensor(data_B).half(), F.to_tensor(label).half()
        else:
